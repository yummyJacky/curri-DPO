...
def train_dpo_curriculum(args: ScriptArguments):
    ...
    # 再次加载SFT adapter作为DPO adapter（用于训练）
    ...
    base_training_args = {
        ...
        "beta": args.beta,
        "num_train_epochs": 1,  # 每个epoch训练一个难度的数据集
    ...
    for epoch, difficulty in enumerate(difficulties, 1):
        ...
        # Update output directory for current epoch and difficulty
        ...
        if epoch < len(difficulties):
            print(f"\n{'='*50}")
            print(f"Preparing for next epoch with {difficulties[epoch]} difficulty")
            # 删除当前的DPO adapter
            model.delete_adapter("DPO")
            # 加载上一轮训练好的adapter作为新的DPO adapter
            DPO_adapter_path = os.path.join(current_output_dir, "DPO")
            print(f"Loading adapter from: {DPO_adapter_path}")
