...
def train_dpo_curriculum(args: ScriptArguments):
    ...
    wandb.init(
        ...
        config={
            ...
            "num_epochs": 3  # 固定为3个epoch，每个epoch对应一个难度
        }
    )
    ...
    for epoch, difficulty in enumerate(difficulties, 1):
        ...
        # 从第二轮开始，将上一轮训练的 DPO adapter 作为 reference adapter
        if epoch > 1:
            model.delete_adapter("reference")
            prev_output_dir = os.path.join(args.output_dir, f"epoch_{epoch-1}_{difficulties[epoch-2]}")
            model.load_adapter(prev_output_dir, adapter_name="reference")
